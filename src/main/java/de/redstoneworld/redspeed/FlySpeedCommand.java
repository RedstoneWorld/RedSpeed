...
public class FlySpeedCommand {
	...
	public boolean sendFlySpeedMsg() {
		...
		player.sendMessage(plugin.getLang("pr�fix") + plugin.getLang("show.fspeed-own", "fspeed", String.valueOf(fs)));
		return true;
	...
	public boolean setDefaultSpeed() {
		...
		player.sendMessage(plugin.getLang("pr�fix") + plugin.getLang("success-fspeed.ownDefaultValue"));
		return true;
	...
	public boolean setSpecificSpeed(float x) {
		...
		player.sendMessage(plugin.getLang("pr�fix")
	...
	public boolean sendFlySpeedMsgOther() {
		...
		float fs = player.getFlySpeed();
		sender.sendMessage(plugin.getLang("pr�fix")
	...
	public boolean setDefaultSpeedOther() {
		...
		player.setFlySpeed(DEFAULT_FLY_SPEED);
		if (sender != player) {
			sender.sendMessage(plugin.getLang("pr�fix") + plugin.getLang("success-fspeed.other", "name", targetName,
	...
	public boolean hasSeePermission() {
		...
		if (!player.hasPermission("rwm.redspeed.fspeed.see")) {
			player.sendMessage(plugin.getLang("pr�fix") + plugin.getLang("noPermission"));
			return false;
		}
		return true;
	}
	...
	public boolean hasSeeOtherPermission() {
		...
		if (!sender.hasPermission("rwm.redspeed.fspeed.see.other")) {
			sender.sendMessage(plugin.getLang("pr�fix") + plugin.getLang("noPermission"));
			return false;
		}
		return true;
	}
	...
	public boolean isOnline() {
		...
		if (this.player == null) {
			sender.sendMessage(plugin.getLang("pr�fix") + plugin.getLang("player-not-found", "name", targetName));
			return false;
		}
		return true;
	}
