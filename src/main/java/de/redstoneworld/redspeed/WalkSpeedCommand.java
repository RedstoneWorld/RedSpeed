...
public class WalkSpeedCommand {
    ...
    /**
     * This method allows to SET the own walk speed value back to the default value, if the executor
    ...
    public boolean sendWalkSpeedMsgOther() {
        ...
        float ws = player.getWalkSpeed();
        sender.sendMessage(plugin.getLang("prefix") + plugin.getLang("show.wspeed-other", "name", targetName,
                "wspeed", String.valueOf(ws)));
        return true;
    ...
    public boolean setDefaultSpeedOther() {
        ...
        player.setWalkSpeed(DEFAULT_WALK_SPEED);
        if (sender != player) {
            sender.sendMessage(plugin.getLang("prefix") + plugin.getLang("success-wspeed.other", "name", targetName,
                    "wspeed", String.valueOf(DEFAULT_WALK_SPEED)));
        }
        player.sendMessage(plugin.getLang("prefix") + plugin.getLang("success-wspeed.ownDefaultValue"));
        return true;
